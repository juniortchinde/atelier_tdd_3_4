...
class MockFileSystem {
    ...
    rmSync(path, options) {
        ...
        if ((options === null || options === void 0 ? void 0 : options.recursive) && this.files[path]) {
    ...
    lstatSync(path) {
        return { isDirectory: () => this.files[path] !== undefined };
    }
    existsSync(path) {
...
describe('FileManager', () => {
    ...
    beforeEach(() => {
        ...
        fileManager = new file_1.FileManager(TEST_DIR, mockFs, mockRandomNameGenerator);
    });
    ...
    it('devrait sélectionner toutes les entrées', () => {
        ...
        expect(fileManager.selectedEntries).toEqual([FILE1, FILE2, SUB_DIR]);
    });
    it('devrait sélectionner les entrées spécifiées', () => {
        fileManager.selectEntries([FILE1, SUB_DIR]);
        expect(fileManager.selectedEntries).toEqual([FILE1, SUB_DIR]);
    });
    ...
    it('devrait lancer une erreur si le répertoire n\'existe pas', () => {
        expect(() => new file_1.FileManager('/non_existent_dir', mockFs, mockRandomNameGenerator)).toThrow('Directory not found: /non_existent_dir');
    });
});
...
describe('FileManager copy with random name conflict', () => {
    ...
    beforeEach(() => {
        ...
        fileManager = new file_1.FileManager(TEST_DIR, fsMock, nameGeneratorMock);
    ...
    it('devrait réessayer avec un nouveau nom si le premier nom aléatoire existe déjà', () => {
        ...
        nameGeneratorMock.generate.mockReturnValueOnce(existingName).mockReturnValueOnce(newName);
        // Simule l'existence du premier répertoire mais pas du second
        fsMock.existsSync.mockImplementation((p) => {
            if (p === TEST_DIR)
                return true; // le répertoire de base
    ...
    it('devrait ajouter un numéro au nom après 10 tentatives échouées', () => {
        ...
        nameGeneratorMock.generate.mockReturnValue(conflictingName);
        fsMock.existsSync.mockImplementation((p) => {
            if (p === TEST_DIR)
                return true;
...
describe('FileManager error handling', () => {
    ...
    it('devrait retourner une liste de fichiers échoués pendant la copie et les garder sélectionnés', () => {
        ...
        expect(fileManager.selectedEntries).toEqual([FILE_TO_FAIL]);
